...
var tracker = {
  ...
  onClick: function(event) {
    ...
    if (tracker.clickcount === 3) { //Change this to 24
      tracker.imagesEl.removeEventListener('click', tracker.onClick);

      var ulEl = document.createElement('ul');
      tracker.resultsEl.appendChild(ulEl);
      console.log('ulEl: ',ulEl);

      for(var j = 0; j < allProducts.length; j++) {
        var liEl = document.createElement('li');
        console.log(liEl);
        liEl.textContent = ('product: ' + allProducts[j].name + ' votes: ' + allProducts[j].votes);
        ulEl.appendChild(liEl);
      }
      console.log('it passed the for loop');
